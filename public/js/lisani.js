        ...
        function showToast(message, type = 'info') {
            ...
            const baseCommon = "fixed top-6 z-50 transform translate-y-0 opacity-100 transition-all duration-300 pointer-events-auto cursor-grab select-none touch-none text-xs font-bold px-5 py-3.5 rounded-2xl flex items-center space-x-2 backdrop-blur-md";
        ...
        function dismissToast(direction) {
            ...
            setTimeout(() => {
                toastBox.className = "fixed top-6 z-50 transform -translate-y-20 opacity-0 transition-all duration-300 pointer-events-none text-white text-xs font-bold px-5 py-3 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex items-center space-x-2 backdrop-blur-md";
                toastBox.style.transform = '';
                toastBox.style.opacity = '';
                toastBox.style.transition = '';
            }, 300);
        ...
        function initLettersGrid(filterQuery = "") {
            ...
            filteredAlphabet.forEach((letter) => {
                ...
                card.className = "theme-card-bg hover:opacity-90 border theme-border rounded-xl p-2.5 text-center cursor-pointer transition transform hover:-translate-y-0.5 active:scale-95 flex flex-col items-center justify-between h-20 shadow-sm";
                card.onclick = () => {
                    playClickSound();
                ...
                card.innerHTML = `
                    <span class="arabic-text text-2xl font-bold theme-primary-color drop-shadow-sm">${letter.ar}</span>
                    <span class="text-[10px] font-bold theme-text-main mt-1">${letter.name}</span>
                    <span class="text-[9px] theme-text-muted font-semibold">${letter.sounds}</span>
                `;
